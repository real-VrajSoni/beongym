...
async function main() {
	...
	const routes: [string, string, string][] = [
		...ADMIN_ROUTES.map(
			([r, expect]) =>
				[r, adminToken, expect] as [string, string, string],
		),
		[`/admin/gyms/${gym.id}`, adminToken, "Gym visits"],
		...GYM_ROUTES.map(
			([r, expect]) =>
				[r, staffToken, expect] as [string, string, string],
		),
		[`/gym/clients/${member.id}`, staffToken, "Member app"],
		[`/gym/plans/${firstPlan.id}`, staffToken, "Revenue collected"],
		...MEMBER_ROUTES.map(
			([r, expect]) =>
				[r, memberToken, expect] as [string, string, string],
		),
		...PUBLIC_ROUTES.map(
			([r, expect]) => [r, "", expect] as [string, string, string],
		),
	];
	...
	const listingRedirect = protectedListing.headers.get("location");
	rows.push({
		route: "/list (anonymous)",
		status: protectedListing.status,
		ok:
			protectedListing.status === 307 &&
			listingRedirect === "/signup?next=/list",
		note:
			protectedListing.status === 307 &&
			listingRedirect === "/signup?next=/list"
				? ""
				: `expected 307 to /signup?next=/list, got ${protectedListing.status} to ${listingRedirect}`,
	});
