...
async function main() {
	...
	const gymA = await db.gym.findUniqueOrThrow({
		where: { code: "IRON-4821" },
	});
	const gymB = await db.gym.findUniqueOrThrow({
		where: { code: "TITAN-2093" },
	});
	...
	try {
		// 1. Anonymous visitors reach nothing. The upgraded account-first flow
		// sends them to signup before any protected page can be reached.
		for (const path of [
			"/admin/overview",
			"/gym/dashboard",
			"/gym/clients",
			...
			add(
				`anonymous -> ${path}`,
				r.status === 307 && (r.location ?? "").includes("/signup"),
				`${r.status} -> ${r.location}`,
			);
		}
		...
		const crossMember = await get(
			`/gym/clients/${memberA.id}`,
			staffBToken,
		);
		add(
			"gym B owner -> gym A member page",
			!crossMember.body.includes(memberA.user.name) &&
				crossMember.body.includes("Page not found"),
		...
		add(
			...
			ownMember.status === 200 &&
				ownMember.body.includes(memberA.user.name),
		...
		add(
			...
			paymentsB.status === 200 &&
				!paymentsB.body.includes(memberA.user.name),
		...
		for (const path of [
			...
			for (const fragment of fragments) {
				if (r.body.includes(fragment))
					noteInApp = `${path} leaked "${fragment.slice(0, 24)}…"`;
			}
		}
		add(
			"staff notes never reach the member app",
			noteInApp === null,
			noteInApp ?? "clean",
		);
		...
		if (leadA) {
			...
			const leaked =
				crossLeads.body.includes(leadA.id) ||
				crossLeads.body.includes(leadA.name);
			add(
		...
		if (classA) {
			...
			add(
				...
				crossClasses.status === 200 &&
					!crossClasses.body.includes(classA.id),
		...
		if (messageA) {
			...
			add(
				...
				crossMessages.status === 200 &&
					!crossMessages.body.includes(messageA.body.slice(0, 40)),
		...
		add(
			...
			classesPage.status === 200 &&
				!classesPage.body.includes(otherMember.user.name),
		...
		add(
			...
			suspended.status === 307 &&
				(suspended.location ?? "").includes("/login"),
		...
		const prospect = await db.user.findFirstOrThrow({
			where: { role: "PROSPECT" },
		});
		...
		add(
			...
			ownerAtStart.status === 307 &&
				(ownerAtStart.location ?? "").includes("/gym"),
		...
		const lapsedToken = await sign({
			...
			gymAccessExpiresAt:
				lapsedGym.accessExpiresAt?.toISOString() ?? null,
		});
		...
		add(
			...
			renew.status === 200 &&
				renew.body.includes("Your access has run out"),
			String(renew.status),
		);
		...
		const lapsedMember = await db.clientProfile.findFirst({
			where: { gymId: lapsedGym.id },
		});
		if (lapsedMember) {
			...
			for (const path of [
				...
				add(
					...
					paused.status === 307 &&
						(paused.location ?? "").includes("/me/paused"),
		...
		add(
			...
			unclaimed.length > 0 &&
				unclaimed.every((g) => !map.body.includes(g.code)),
			unclaimed
		...
		add(
			...
			unclaimed.length > 0 &&
				unclaimed.every(
					(g) => g._count.users === 0 && g._count.members === 0,
				),
			unclaimed
				.map((g) => `${g.code}:${g._count.users}/${g._count.members}`)
				.join(" "),
		);
		if (unclaimed[0]) {
		...
		// 12c. Listing is purchase-protected: a visitor must register before a
		// Dodo checkout session can be created. Historical accountless listings
		// remain readable but are no longer a permitted purchase path.
		const listPage = await get("/list");
		add(
			"anonymous visitor must sign up before listing payment",
			listPage.status === 307 &&
				listPage.location === "/signup?next=/list",
			`${listPage.status} -> ${listPage.location}`,
		);
		...
		const orphanGyms = accountless.length
			...
			: [];
		add(
			"historical accountless listings have no accounts",
			orphanGyms.every(
				(g) => g._count.users === 0 && g._count.members === 0,
			),
			orphanGyms
		...
		for (const path of ["/gym/staff", "/gym/billing"]) {
			...
			add(
				...
				r.status === 307 &&
					(r.location ?? "").includes("/gym/dashboard"),
				`${r.status} -> ${r.location}`,
			);
		}
