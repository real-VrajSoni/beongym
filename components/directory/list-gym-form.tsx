		...
		<div>
			...
			{step === "choose" ? (
              ...
              still in the form when it submits. */}
					<div
						className={cn("space-y-5", step !== "gym" && "hidden")}
					>
						<FormGrid>
							<FormField
								label="Gym name"
						...
						<div className="rounded-xl border border-[var(--border)] bg-[var(--surface-muted)] p-4">
							...
							<ImageUpload
								...
								accentColor={accent}
								fallback={
									name.slice(0, 2).toUpperCase() || "GY"
								}
							/>
							<p className="mt-3 border-t border-[var(--border)] pt-3 text-[12px] text-muted-foreground">
								This is your pin on the globe. Without one,
								members see your initials.
							</p>
						</div>
						...
						<div className="space-y-5">
							...
							<div className="rounded-xl border border-[var(--border)] bg-[var(--surface-muted)] px-4 py-3">
								<p className="text-[12.5px] leading-relaxed text-muted-foreground">
									Payment is completed on the secure Dodo
									Payments checkout. Your listing goes live
									after Dodo confirms the payment; taxes are
									calculated there from your billing country.
								</p>
							</div>
							...
							<div className="flex flex-wrap items-center justify-between gap-3 border-t border-[var(--border)] pt-5">
								...
								<Button
									...
									loading={pending}
								>
									<CreditCard /> Pay {formatUsd(plan.price)}{" "}
									and go live
								</Button>
							</div>
						</div>
				...
				<li key={key} className="flex items-center gap-2">
					<span
						className={cn(
							...
							i > index &&
								"border border-[var(--border-strong)] text-muted-foreground",
						)}
					>
					...
					{i < items.length - 1 ? (
						<span
							className="mx-1 h-px w-6 bg-[var(--border-strong)]"
							aria-hidden
						/>
					) : null}
				</li>
	...
	return (
		<div className="mt-6">
					...
					return (
						...
						>
							...
							<div className="mt-3 flex items-baseline gap-2">
								<p className="text-[26px] leading-none font-semibold">
									{formatUsd(plan.price)}
								</p>
							...
							<p className="mt-0.5 text-[12px] text-muted-foreground">
								{formatUsd(
									Math.round(perMonth(plan.key) * 100) / 100,
								)}{" "}
								a month
							</p>
			...
			<p className="mt-4 text-[12.5px] leading-relaxed text-muted-foreground">
				Both plans are the same product — members, payments, attendance,
				renewals, enquiries, classes and your listing on the map. Only
				the length differs. There is no free tier: everything on the
				globe belongs to a gym that paid to be there.
			</p>
		</div>
	);
}
		...
		<div className="mt-6">
				...
				<div className="flex items-start gap-3">
					...
					<div>
						...
						<p className="mt-1 text-[13px] text-muted-foreground">
							...
							. Keep it — it is how you and your members find your
							gym.
						</p>
						<Link
							href={`/gyms/${code}`}
			...
			{attached ? (
				<div className="mt-5 rounded-2xl border border-[var(--brand)]/30 bg-[var(--brand)]/[0.06] p-5">
					...
					<p className="mt-1 text-[13px] text-muted-foreground">
						Add your logo, a line about the gym, your photos, your
						programmes and the links your pin sends people to.
						Prices stay hidden until you publish them.
					</p>
					<Link
						href="/gym/settings"
						className="mt-3 inline-flex items-center gap-1.5 rounded-lg bg-[var(--brand)] px-3.5 py-2 text-[13px] font-medium text-[var(--brand-foreground)] hover:bg-[var(--brand-hover)]"
				...
				>
					...
					<p className="mt-1 mb-4 text-[13px] text-muted-foreground">
						You&rsquo;ve paid for a store; you need a login to build
						it. Set a password and you&rsquo;ll land straight in the
						setup.
					</p>
					<FormError message={error} />
					<FormGrid>
