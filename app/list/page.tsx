...
export default async function ListGymPage({
	...
	// A payment can only start from a registered prospect account. This page is
	// public in the URL but not a checkout bypass: signup establishes the account
	// and its verified email before any Dodo session can be created.
	const session = await getValidSession();
	if (!session) redirect("/signup?next=/list");
	if (session.role !== "PROSPECT") redirect(homeFor(session.role));
		...
		<div className="min-h-dvh bg-[var(--mk-bg)] text-[var(--mk-fg)]">
			...
			>
				<div className="mx-auto max-w-6xl px-6 py-14">
					<p className="inline-flex items-center gap-1.5 rounded-full border border-[var(--brand)]/30 bg-[var(--brand)]/10 px-2.5 py-1 text-[11.5px] font-medium text-[var(--brand)]">
						<Sparkles className="size-3" /> ${ENTRY_PRICE} a month,
						or ${planByKey("ANNUAL").price} for the year
					</p>
			...
			<main className="mx-auto max-w-3xl px-6 py-12">
				...
				<p className="mt-10 border-t border-[var(--mk-border)] pt-6 text-[13px] leading-relaxed text-[var(--mk-fg-subtle)]">
					...
					straight away — your store, members, attendance, the diary,
					plans and payments. Already on the map and want to take over
					the listing?{" "}
					<Link
						href="/claim"
						className="font-medium text-[var(--brand)] hover:underline"
