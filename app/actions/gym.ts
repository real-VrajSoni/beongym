...
export async function updateGymProfileAction(
	formData: FormData,
): Promise<ActionResult> {
	return guard(async () => {
		const session = await requireOwner();
		const parsed = gymProfileSchema.safeParse(
 ...
 */
export async function purchaseAccessAction(
	planKey: string,
): Promise<ActionResult> {
	return guard(async () => {
		const session = await requireOwner();
		...
		if (before.tier === "ELITE") {
			return {
				ok: false,
				error: "You already have lifetime access — there is nothing to renew.",
			};
		}
		...
		const gym = await db.gym.findUniqueOrThrow({
			where: { id: session.gymId },
			select: { name: true, city: true, currency: true, country: true },
		});
