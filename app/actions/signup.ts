...
const createAccountSchema = z.object({
	...
	/** A small allow-list prevents this registration redirect from becoming an
      open redirect while preserving the protected public-listing flow. */
	next: z.enum(["/list"]).optional(),
});
...
export async function createAccountAction(
	...
	const result = await guard(async () => {
		...
		if (d.claim)
			destination = `/gyms/${encodeURIComponent(d.claim.toUpperCase())}/claim`;
		else if (d.next) destination = d.next;
