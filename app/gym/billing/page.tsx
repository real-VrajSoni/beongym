...
import { ENTRY_PRICE, accessState, daysLeft, planByKey, planForTier } from "@/lib/platform-plans";
    ...
    <>
      ...
      <div className="mb-5 grid grid-cols-2 gap-3 sm:gap-4 lg:grid-cols-4">
        ...
        <StatCard
          ...
          value={lifetime ? "Never" : formatDate(gym.accessExpiresAt ?? new Date())}
          icon={Timer}
          hint={lifetime ? "lifetime member" : `${remaining} days remaining`}
        />
        <StatCard
          label="Members"
        ...
        />
        <StatCard label="With BeOnGym since" value={formatDate(gym.createdAt)} />
      </div>

      <div className="mb-3 flex flex-wrap items-baseline justify-between gap-2">
        <h2 className="text-[15px] font-semibold">{lifetime ? "Your plan" : "Renew or upgrade"}</h2>
