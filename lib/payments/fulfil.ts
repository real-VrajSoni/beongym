...
export async function fulfilOrder(
		...
		const gym = await tx.gym.create({
			data: {
				...
				businessType: (str(meta, "businessType") ??
					DEFAULT_BUSINESS_TYPE) as BusinessTypeKey,
				...
				amenities: Array.isArray(meta.amenities)
	...
	paymentLog("info", "order.fulfilled", {
		orderId: order.id,
		gymId,
		kind: order.kind,
		planKey,
